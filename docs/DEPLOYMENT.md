...
| Deploy targets ενεργά | `hosting`, `firestore:rules`, `firestore:indexes` ✅ |
| `storage` (rules) | ⏸️ αναμονή — το firebase-tools δεν εντοπίζει ακόμη το bucket |
...
```
Ο ρόλος *Cloud Datastore Index Admin* προστέθηκε, οπότε τα indexes ανεβαίνουν.
Το `storage` παρακρατείται προσωρινά (δες παρακάτω). Το bucket έχει δηλωθεί ρητά
στο `firebase.json` (`app-koinoxriston.firebasestorage.app`).
