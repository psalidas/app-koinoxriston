...
Υπάρχει και **«Επαναποστολή πρόσκλησης»** (callable `resendInvite`) στη λίστα
χρηστών, για χρήστες που φτιάχτηκαν πριν ρυθμιστούν τα κλειδιά ή για επανάληψη.

**Προϋπόθεση: Blaze plan** (για Functions Gen2).

1. **API keys → GitHub Secrets** (repo → Settings → Secrets and variables →
   Actions → New repository secret):
   - **`BREVO_API_KEY`** — Brevo → SMTP & API → API Keys.
   - **`SMSTO_API_KEY`** — SMS.to → API.

   Το workflow τα γράφει σε `functions/.env` (gitignored) και ανεβαίνουν ως env
   vars των Cloud Functions κατά το deploy. Δεν μπαίνουν ποτέ σε Firestore/frontend.
...
3. **Ρυθμίσεις στην εφαρμογή**: Διαχείριση → **Προσκλήσεις**. Όρισε:
   ενεργό/ανενεργό, διεύθυνση εφαρμογής, **Email αποστολέα** (verified στο Brevo),
   όνομα αποστολέα, **Sender ID** SMS. (Γράφεται στο Firestore `settings/invites`.)
   Αν λείπει το «Email αποστολέα», οι email προσκλήσεις παραλείπονται (SMS ανεπηρέαστο).

4. **Deploy των functions**: γίνεται **αυτόματα από το GitHub Actions** (ξεχωριστό
   βήμα «Deploy functions»). Ο deploy service account χρειάζεται τους ρόλους:
   *Cloud Functions Admin*, *Cloud Run Admin*, *Artifact Registry Admin*,
   *Service Account User* (+ ενεργά τα APIs cloudfunctions/cloudbuild/run/
   artifactregistry/eventarc). Εναλλακτικά χειροκίνητα:
   ```bash
   firebase deploy --only functions --project app-koinoxriston
   ```
