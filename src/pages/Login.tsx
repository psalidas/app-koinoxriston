      ...
      <div className="w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        {!isFirebaseConfigured && (
          <div className="mb-4 rounded-md bg-amber-50 p-3 text-xs text-amber-700">
            Το Firebase δεν έχει ρυθμιστεί ακόμη. Συμπληρώστε το αρχείο{' '}
            <code>.env</code> (δείτε <code>docs/SETUP.md</code>).
          </div>
        )}

        {error && (
          <div className="mb-4 rounded-md bg-red-50 p-3 text-xs text-red-700">{error}</div>
        )}
        ...
        {sent ? (
          <div className="rounded-md bg-green-50 p-3 text-sm text-green-700">
            Στείλαμε σύνδεσμο εισόδου στο <strong>{email}</strong>. Ανοίξτε το email
            σας από αυτή τη συσκευή.
          </div>
        ) : (
